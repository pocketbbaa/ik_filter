...
class CJKSegmenter implements ISegmenter {
    ...
    public void analyze(AnalyzeContext context, FilterType filterType) {
        if (CharacterUtil.CHAR_USELESS != context.getCurrentCharType()) {
            ...
            //*********************************
            //再对当前指针位置的字符进行单字匹配
            Hit singleCharHit = new Hit();
            if (filterType.equals(FilterType.SENSITIVE_WORDS)) {
                singleCharHit = Dictionary.getSingleton().matchInSWDict(context.getSegmentBuff(), context.getCursor(), 1);
            }
            if (filterType.equals(FilterType.KEY_WORDS)) {
                singleCharHit = Dictionary.getSingleton().matchInKWDict(context.getSegmentBuff(), context.getCursor(), 1);
            }
